...
IHE primarily focuses on interoperability and the exchange of information between systems, rather than how data is
...
that, which isn't a weird thing, right? You can't force healthcare vendors to conform to _your idea_ of an information
...
interoperability
is built in it's core rather than an add-on.
...
Immediate ares of exploration were MHD, mXDE and QEDm profiles and how they work with openEHR foundation underneath. We
furthermore decided to focus on EHDS and IPS use case, to make it more tangible.

### QEDm

..stands for Query Existing Data (mobile - signaling that's it follows a RESTful pattern). It describes behavior of a
client
...
what are responsibilities of each actors. When one of the actors is an openEHR CDR (for server side component), there
are various
...
for openEHR CDR bundled with a FHIRConnect engine (such as openFHIR). mXDE and QEDm profiles could further specify how
...
you
have a **standard, reproducable and scalable stack based entirely on open standards**. A vendor neutral architecture where
both
information model as well as exchange is standardized, community driven and interopeable out of the box, easily scalable
in all
...
complicance
of a vendor is the next best thing, because it still promises some sort of interopreability and solution to a vendor
lock-in,
...
proprietary system, sufficing the bare minimum to get the _ribbon_ of being interoperable? Having that kind of _ribbon_
as
a vendor an IHE proving that kind of a capability is next level and would truly mean something to a hospital or a care
provider.

## Architecture

Couple levels deeper, this is how data flow would look like when openEHR acts as a Data Source in an architecture of IHE
...
about an ["EHR Sandwich"](https://www.linkedin.com/in/seref-arikan).
